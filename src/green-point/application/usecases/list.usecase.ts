...
class ListGreenPointsUseCase {
  ...
  async exec(offset: number, limit: number, entityId?: string, includeInactive?: boolean): Promise<GreenPointEntity[]> {
    const greenPoints = await this.repository.list(offset, limit, entityId, includeInactive)
    if (greenPoints == null) {
