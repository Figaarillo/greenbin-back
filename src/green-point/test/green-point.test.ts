...
import { createEntity, createGreenPoint, GREEN_POINT_FIXTURE } from '../../shared/test/test-helpers'
...
describe('GreenPoint — integration tests', () => {
  ...
  describe('POST /api/green-point', () => {
    ...
    it('devuelve 409 con coordenadas duplicadas', async () => {
      await createGreenPoint(app, entityId)
      const res = await app.inject({
        method: 'POST',
        url: '/api/green-point',
    ...
    it('el punto verde aparece en la entidad al popular', async () => {
      await createGreenPoint(app, entityId)
      const res = await app.inject({ method: 'GET', url: `/api/entity/populate?id=${entityId}&with=greenPoints` })
  ...
  describe('GET /api/green-point/:id', () => {
    it('obtiene un punto verde por id', async () => {
      const gp = await createGreenPoint(app, entityId)
      const res = await app.inject({ method: 'GET', url: `/api/green-point/${gp.id}` })
      expect(res.statusCode).toBe(200)
      expect(res.json().data.name).toBe(GREEN_POINT_FIXTURE.name)
    })

    it('devuelve 404 con id inexistente', async () => {
      const res = await app.inject({ method: 'GET', url: '/api/green-point/00000000-0000-0000-0000-000000000000' })
      expect(res.statusCode).toBe(404)
    })
  })
  ...
  describe('GET /api/green-point', () => {
    it('lista puntos verdes con paginación', async () => {
      ...
      const res = await app.inject({ method: 'GET', url: '/api/green-point?offset=0&limit=10' })
      expect(res.statusCode).toBe(200)
  ...
  describe('DELETE /api/green-point/:id', () => {
    it('elimina un punto verde existente', async () => {
      const gp = await createGreenPoint(app, entityId)
      const res = await app.inject({ method: 'DELETE', url: `/api/green-point/${gp.id}` })
      expect(res.statusCode).toBe(200)
    })

    it('el GET devuelve 404 luego del DELETE', async () => {
      const gp = await createGreenPoint(app, entityId)
      await app.inject({ method: 'DELETE', url: `/api/green-point/${gp.id}` })
      const res = await app.inject({ method: 'GET', url: `/api/green-point/${gp.id}` })
      expect(res.statusCode).toBe(404)
    })
  })
