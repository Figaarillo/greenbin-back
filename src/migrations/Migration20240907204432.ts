...
export class Migration20240907204432 extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(
      'create table "responsible_entity" ("id" uuid not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "firstname" varchar(255) not null, "lastname" varchar(255) not null, "username" varchar(255) not null, "email" varchar(255) not null, "password" varchar(255) not null, "dni" int not null, "phone_number" varchar(255) not null, constraint "responsible_entity_pkey" primary key ("id"));'
    )
    this.addSql(
    ...
    this.addSql(
      'create table "reward_partner_entity" ("id" uuid not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "name" varchar(255) not null, "username" varchar(255) not null, "address" varchar(255) not null, "cuit" varchar(255) not null, "email" varchar(255) not null, "password" varchar(255) not null, "phone_number" varchar(255) not null, constraint "reward_partner_entity_pkey" primary key ("id"));'
    )
    this.addSql(
      'alter table "reward_partner_entity" add constraint "reward_partner_entity_name_unique" unique ("name");'
    )
    this.addSql(
      'alter table "reward_partner_entity" add constraint "reward_partner_entity_username_unique" unique ("username");'
    )
    this.addSql(
      'alter table "reward_partner_entity" add constraint "reward_partner_entity_cuit_unique" unique ("cuit");'
    )
    this.addSql(
      'alter table "reward_partner_entity" add constraint "reward_partner_entity_email_unique" unique ("email");'
    )
